...
// --- createTimeEntry overwrite behavior ---

describe("createTimeEntry overwrite", () => {
  it("overwrites existing entry with same project+task+date", async () => {
    await provider.createTimeEntry("emp1", makeEntry({ description: "task 1", taskId: "t1" }));

    const result = await provider.createTimeEntry(
      "emp1",
      makeEntry({ description: "- task 1\n- task 2", taskId: "t1", minutes: 90 })
    );

    // App is source of truth — overwrites, not merges
  ...
  it("does not match across different tasks", async () => {
    await provider.createTimeEntry("emp1", makeEntry({ description: "task 1", taskId: "t1" }));
